...
pacman::p_load(tidyr,
               ...
               zoo,
               rvest)
if(!require("dkstat")) install_github("rOpenGov/dkstat"); library(dkstat)
...
df_udbud %>% 
  ...
  dplyr::mutate(floor_date=lubridate::floor_date(DATE,unit = "year")) %>% 
  group_by(PROPERTY_TYPE,SUPPLY_TYPE,ZIP) %>% 
  arrange(PROPERTY_TYPE,SUPPLY_TYPE,ZIP,DATE)  %>% 
  dplyr::mutate(month_over_month=VALUE/lag(VALUE)-1) %>% 
  dplyr::mutate(year_=factor(year(DATE)),
                month_=month(DATE)) %>% 
  ggplot(.,aes(x=month_,y=month_over_month,group=month_)) +
  geom_boxplot() +
  geom_smooth() +
  scale_x_continuous(breaks = seq(12)) +
  scale_y_continuous(labels = scales::percent) +
  facet_wrap(ZIP~SUPPLY_TYPE,scales="free") +
  theme(legend.position="bottom")
```
...
df %>% 
  ...
  dplyr::mutate(floor_date=lubridate::floor_date(DATE,unit = "year")) %>% 
  group_by(PROPERTY_TYPE,SALES_TYPE,ZIP) %>% 
  arrange(PROPERTY_TYPE,SALES_TYPE,ZIP,DATE) %>% 
  dplyr::mutate(month_over_month=VALUE/lag(VALUE)-1) %>% 
  dplyr::mutate(year_=factor(year(DATE)),
                month=month(DATE)) %>% 
  ggplot(.,aes(x=month,y=month_over_month,group=month)) +
  geom_boxplot() +
  geom_smooth() +
  scale_x_continuous(breaks = seq(12)) +
  scale_y_continuous(labels = scales::percent) +
  facet_wrap(ZIP~SALES_TYPE,scales="free") +
  theme(legend.position="bottom")
```
