...
df_DK = openxlsx::read.xlsx(xlsxFile = "obligationsrente_fida_uge11-2022.xlsx",startRow = 3)
...
df_predict_DFF %>% 
  ...
  filter(date %in% seq.Date(from = Sys.Date(),to=Sys.Date()+months(13),by = "3 months")) %>% 
  gather(pred,val,pred_DFF:pred_MORTGAGE30US) %>% 
