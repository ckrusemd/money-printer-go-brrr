...
dst_get_data <- dkstat::dst_get_data
dst_meta <- dkstat::dst_meta
```
...
finans_page = xml2::read_html(source_url)
current_url = finans_page %>%
  rvest::html_elements("a[href*='.xlsx']") %>%
  rvest::html_attr("href") %>%
  .[!is.na(.)] %>%
  .[grepl("obl[-_]rente|obligationsrente", ., ignore.case = TRUE)] %>%
  dplyr::last()
if (length(current_url) == 0) current_url = NA_character_
df_interest = data.frame(
  date = as.Date(character()), value = numeric(), series_id = character()
)
if (!is.na(current_url) && nzchar(current_url)) {
  xlsx_url = xml2::url_absolute(current_url, source_url)
  df_interest = tryCatch({
    interest = openxlsx::read.xlsx(xlsxFile = xlsx_url, startRow = 1, check.names = TRUE)
    year_name = names(interest)[grepl("^År$|^Ar$", names(interest))][1]
    week_name = names(interest)[grepl("^Uge$", names(interest))][1]
    short_name = names(interest)[grepl("^Kort[.]rente$|^Kort.rente", names(interest))][1]
    long_name = names(interest)[grepl("^Lang[.]rente$|^Lang.rente", names(interest))][1]
    euro_name = names(interest)[grepl("Kort.*euro.*rente", names(interest), ignore.case = TRUE)][1]
    required_columns = c(year_name, week_name, short_name, long_name)
    if (anyNA(required_columns)) stop("Finans Danmark spreadsheet columns changed")
    interest = interest %>% fill(dplyr::all_of(c(year_name, week_name)), .direction = "down")
    interest$Date = as.Date(
      paste(as.integer(interest[[year_name]]), sprintf("%02d", as.integer(interest[[week_name]])), 1, sep="-"),
      "%Y-%U-%u"
    )
    rate_data = interest %>%
      dplyr::transmute(
        Date = Date,
        `Kort.rente` = .data[[short_name]],
        `Lang.rente` = .data[[long_name]],
        `Kort.euro.rente` = if (!is.na(euro_name)) .data[[euro_name]] else NA_real_
      )
    rate_data %>%
    ...
    warning("Finans Danmark data unavailable; continuing without it: ", error$message)
    df_interest
  })
} else {
  warning("Finans Danmark download link unavailable; continuing without it")
}
