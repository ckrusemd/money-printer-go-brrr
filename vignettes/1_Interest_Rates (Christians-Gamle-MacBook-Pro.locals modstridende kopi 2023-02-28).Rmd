...
filename_ = paste0("Data/",list.files("FedRate",path = "Data"))
...
rate_probs = do.call("rbind",lapply(seq(2,length(cols_to_loop)),function(cols_) {
  ...
  dplyr::mutate(Meeting=parse_date(Meeting,"%d %b %Y")) %>% 
...
meetings_with_highest_rate = rate_probs %>% 
    filter(Val>0) %>% 
    filter(!is.na(Val)) %>% 
    arrange(Date) %>% 
    group_by(Date) %>% 
...
rate_probs %>% 
  inner_join( meetings_with_highest_rate ) %>% 
  arrange(Date) %>% 
  filter(Val>0) %>% 
  dplyr::mutate(Rate=factor(Rate)) %>% 
...
p1 = rate_probs %>% 
  filter(Val>0) %>% 
  group_by(Date,Meeting) %>% 
  dplyr::summarize(prob_weight=sum(Rate*Val)) %>% 
  group_by(Date) %>% 
  filter(prob_weight==max(prob_weight)) %>% 
  ggplot(.,aes(x=Date,y=prob_weight)) +
  geom_line() +
  scale_x_date(date_breaks = "1 month",date_labels = "%Y %b",limits=c(dmy("01-01-2022"),dmy("01-01-2024"))) +
  labs(x=NULL,y=NULL) +
  theme_bw() +
  theme(axis.text.x=element_text(angle=45,hjust=1)) +
  scale_fill_gradient2(low="black",high="green")  +
  geom_line(data=df_dff %>% filter(date>=dmy("01-01-2022")),aes(x=date,y=value)) +
...
lm_diff = rate_probs %>% 
  ...
  group_by(Date) %>% 
  filter(prob_weight==max(prob_weight)) %>% 
  inner_join(df_dff , by = c("Date"="date")) %>% 
  dplyr::mutate(diff=prob_weight-value) %>% 
  lm(data=.,diff~Date)

df_preddiff = data.frame(Date=seq.Date(from = dmy("01-01-2021"),to = dmy("01-01-2024"),by = "1 day")) %>% 
  dplyr::mutate(predicted=as.data.frame(predict(lm_diff,.,interval="prediction"))) %>% 
  as.data.frame()
df_preddiff = df_preddiff$predicted %>% bind_cols(data.frame(Date=df_preddiff$Date))
...
p2 = rate_probs %>% 
  ...
  theme(axis.text.x=element_text(angle=45,hjust=1)) +
  scale_fill_gradient2(low="black",high="green") +
  scale_y_continuous(limits = c(-1,4),breaks=seq(0,10,by=0.5)) +
  geom_hline(yintercept = 0,linetype=2) +
  geom_line(data=df_preddiff,aes(x=Date,y=fit)) +
  geom_ribbon(data=df_preddiff,aes(y=fit,ymin=lwr,ymax=upr),alpha=0.3) +
  add_fomc_meeting_dates()
...
## Terminal Rate and 30 Year Mortgage

```{r}

df_mortgage_dff = df_MORTGAGE30US %>% 
  inner_join(df_DFF,by=c("date"="date")) %>% 
  dplyr::mutate(diff_mortgage_dff=value.x-value.y) %>% 
  dplyr::select(date,diff_mortgage_dff)
...
p3 = rate_probs %>% 
  ...
  group_by(Date) %>% 
  filter(prob_weight==max(prob_weight)) %>% 
  inner_join(df_MORTGAGE30US,by=c("Date"="date")) %>% 
  dplyr::mutate(diff=value-prob_weight) %>% 
  ggplot(.,aes(x=Date,y=diff)) +
  geom_line() +
  # scale_x_date(date_breaks = "1 month",date_labels = "%Y %b",limits=c(dmy("01-01-2022"),dmy("01-01-2024"))) +
  labs(x=NULL,y=NULL) +
  ...
  theme(axis.text.x=element_text(angle=45,hjust=1)) +
  scale_fill_gradient2(low="black",high="green") +
  scale_y_continuous(limits = c(-0.5,NA),breaks=seq(0,10,by=0.25)) +
  geom_hline(yintercept = 0,linetype=2) +
  geom_smooth(method="lm") 

p4 = ggplot(df_mortgage_dff,aes(x=date,y=diff_mortgage_dff)) + 
  geom_line() +
  scale_x_date(date_breaks = "2 years",date_labels = "%Y") +
  scale_y_continuous(limits=c(0,7),breaks = seq(0,10,by=0.5)) +
  labs(x=NULL,y=NULL) +
  theme_bw() +
  geom_vline(xintercept = dmy("01-07-2006"))+
  geom_vline(xintercept = dmy("01-06-2022"))
...
fredr(series_id = "WILL5000INDFC") %>% 
  ...
  bind_rows(df_dff) %>% 
  crossing(data.frame(type=c("Pause","Pause","Pause","Cut","Cut","Cut"),
