...
if (!requireNamespace("pacman", quietly = TRUE)) {
  stop("The locked publication environment is missing pacman.")
}
pacman::p_load(fredr,
               dplyr,
               tidyr,
...
get_ecb_data <- function(key, attempts = 4) {
  last_error <- NULL
  for (attempt in seq_len(attempts)) {
    result <- tryCatch(
      ecb::get_data(key = key),
...
## ECB data availability {#test-ecb}

```{r ecb1}
get_ecb_data(key = glue("FM.D.U2.EUR.4F.KR.DFR.LEV"))
```


## Treasury yield data {#normal}
```{r }
df_yieldcurve = fredr_cached_many(c(
  "DGS1", "DGS2", "DGS3", "DGS5", "DGS7", "DGS10", "DGS20",
  "DGS30", "DGS1MO", "DGS3MO", "DGS6MO", "DFF", "MORTGAGE30US"
))
...
xlsx_url = finansdanmark_xlsx_url(source_url)
...
# Effective dates and deposit-facility rates from the ECB's official history:
# https://www.ecb.europa.eu/stats/policy_and_exchange_rates/key_ecb_interest_rates/html/index.en.html
ecb_rate_changes = tibble::tribble(
  ~effective_date, ~deposit_rate, ~change_bps,
  "2011-04-13",  0.50,   25,
  "2011-07-13",  0.75,   25,
  "2011-11-09",  0.50,  -25,
  "2011-12-14",  0.25,  -25,
  ...
  dplyr::mutate(effective_date = ymd(effective_date))
...
df_interest %>%
  ...
  geom_vline(data = ecb_rate_changes,
             aes(xintercept = effective_date, color = change_bps > 0),
...
```{r}
df_yieldcurve =
  fredr_cached(series_id = "DGS1") %>%
  bind_rows(fredr_cached(series_id = "DGS2")) %>%
  bind_rows(fredr_cached(series_id = "DGS3")) %>%
  bind_rows(fredr_cached(series_id = "DGS5")) %>%
  bind_rows(fredr_cached(series_id = "DGS7")) %>%
  bind_rows(fredr_cached(series_id = "DGS10")) %>%
  bind_rows(fredr_cached(series_id = "DGS20")) %>%
  bind_rows(fredr_cached(series_id = "DGS30")) %>%
  bind_rows(fredr_cached(series_id = "DGS1MO")) %>%
  bind_rows(fredr_cached(series_id = "DGS3MO")) %>%
  bind_rows(fredr_cached(series_id = "DGS6MO")) %>%
  bind_rows(fredr_cached(series_id = "DFF")) %>%
  bind_rows(fredr_cached(series_id = "MORTGAGE30US"))
...
```{r}
df_INFLATION =
  fredr_cached(series_id = "CPIAUCSL") %>%
  arrange(date) %>%
  dplyr::mutate(value=100*((value/lag(x = value,n = 12))-1))
df_CSUSHPINSA =
  fredr_cached(series_id = "CSUSHPINSA") %>%
  arrange(date)
df_M2SL =
  fredr_cached(series_id = "M2SL") %>%
  arrange(date)
df_UMCSENT =
  fredr_cached(series_id = "UMCSENT") %>%
  arrange(date)
df_USREC =
  fredr_cached(series_id = "USREC") %>%
  arrange(date)
...
# Hyperparameters are fixed here. Model quality is assessed below with a
# chronological expanding-window backtest, not random cross-validation.
...
# Add absolute levels. Spreads alone cannot distinguish a parallel shift in
# the curve, even though such a shift materially changes every target yield.
df_level_features = df_yieldcurve %>%
  dplyr::select(date,series_id,value) %>%
  dplyr::mutate(series_id=paste0("level_",series_id)) %>%
  tidyr::spread(series_id,value)

add_level_features = function(data) {
  data %>%
    dplyr::left_join(df_level_features,by="date")
}

df_predict_DFF = add_level_features(df_predict_DFF)
df_predict_DGS2 = add_level_features(df_predict_DGS2)
df_predict_DGS10 = add_level_features(df_predict_DGS10)
df_predict_MORTGAGE30US = add_level_features(df_predict_MORTGAGE30US)
df_predict_DK_Short = add_level_features(df_predict_DK_Short)
df_predict_DK_Long = add_level_features(df_predict_DK_Long)
df_predict_CPIAUCSL = add_level_features(df_predict_CPIAUCSL)
df_predict_CSUSHPINSA = add_level_features(df_predict_CSUSHPINSA)
df_predict_M2SL = add_level_features(df_predict_M2SL)
df_predict_UMCSENT = add_level_features(df_predict_UMCSENT)
df_predict_USREC = add_level_features(df_predict_USREC)
...
## Honest Walk-Forward Validation

The random forest is evaluated as a one-year-ahead forecasting model. Each
backtest fit uses only observations whose one-year-ahead outcome was already
known at the forecast origin. The test origins are quarterly to reduce overlap
and deployment time. "Persistence" is the deliberately difficult baseline that
assumes the yield one year from now will equal its current level.

```{r rf-walk-forward-validation}

rf_targets = list(
  DFF = list(data=df_predict_DFF,current="level_DFF"),
  DGS2 = list(data=df_predict_DGS2,current="level_DGS2"),
  DGS10 = list(data=df_predict_DGS10,current="level_DGS10"),
  MORTGAGE30US = list(data=df_predict_MORTGAGE30US,current="level_MORTGAGE30US")
)

walk_forward_rf = function(target_name,target_spec,
                           initial_years=15,test_every_months=3) {
  model_data = target_spec$data %>%
    dplyr::mutate(target_date=date+years(1),
  ...
  feature_names = setdiff(names(model_data),c("date","target_date","value"))
  model_data = model_data %>%
    dplyr::filter(stats::complete.cases(dplyr::select(.,dplyr::all_of(feature_names))))

  first_origin = min(model_data$date)+years(initial_years)
  origins = model_data %>%
    dplyr::filter(date>=first_origin,!is.na(value)) %>%
    dplyr::filter(lubridate::month(date) %in% seq(1,12,by=test_every_months)) %>%
    dplyr::pull(date)

  results = lapply(seq_along(origins),function(i) {
    origin = origins[i]
    train = model_data %>%
      dplyr::filter(target_date<=origin,!is.na(value))
    test = model_data %>%
      dplyr::filter(date==origin,!is.na(value))

    if (nrow(train)<120 || nrow(test)!=1) return(NULL)

    set.seed(1971+i)
    fit = randomForest::randomForest(
      x=data.frame(train[,feature_names,drop=FALSE]),
      y=train$value,
      ntree=num_trees,
      mtry=min(3,length(feature_names))
    )
    ...
    data.frame(
      ...
      actual=test$value,
      random_forest=as.numeric(predict(fit,data.frame(test[,feature_names,drop=FALSE]))),
      persistence=as.numeric(test[[target_spec$current]]),
      curve_regime=ifelse(test$level_DGS10-test$level_DGS2<0,"Inverted","Normal")
    )
  })
...
rf_backtest = dplyr::bind_rows(lapply(names(rf_targets),function(target_name) {
  walk_forward_rf(target_name,rf_targets[[target_name]])
})) %>%
  dplyr::arrange(target,origin) %>%
  dplyr::group_by(target) %>%
  dplyr::mutate(
    error=random_forest-actual,
    persistence_error=persistence-actual,
    interval_half_width=vapply(seq_along(error),function(i) {
      previous_errors=abs(error[seq_len(i-1)])
      if (length(previous_errors)<8) return(NA_real_)
      as.numeric(stats::quantile(previous_errors,0.80,na.rm=TRUE,type=8))
    },numeric(1)),
    lower_80=random_forest-interval_half_width,
    upper_80=random_forest+interval_half_width
  ) %>%
  dplyr::ungroup()

rf_metrics = rf_backtest %>%
  dplyr::group_by(target) %>%
  dplyr::summarise(
    observations=dplyr::n(),
    rf_rmse=sqrt(mean(error^2)),
    persistence_rmse=sqrt(mean(persistence_error^2)),
    rmse_skill=1-rf_rmse/persistence_rmse,
    rf_mae=mean(abs(error)),
    persistence_mae=mean(abs(persistence_error)),
    interval_80_coverage=mean(actual>=lower_80 & actual<=upper_80,na.rm=TRUE),
    .groups="drop"
  )
...
knitr::kable(
  ...
  caption="Expanding-window one-year forecast performance. Positive skill means the forest beats persistence."
)

```

```{r rf-walk-forward-chart, fig.height=8}

rf_backtest %>%
  dplyr::select(target,target_date,actual,random_forest,persistence) %>%
  tidyr::gather(model,value,actual:persistence) %>%
  ggplot(.,aes(x=target_date,y=value,color=model))+
  geom_line(linewidth=0.45)+
  facet_wrap(~target,scales="free_y",ncol=1)+
  theme_money_printer_go_brrr(base_size=12)+
  labs(title="Genuinely Out-of-Sample One-Year Yield Forecasts",
       subtitle="Expanding-window random forest versus a no-change persistence benchmark",
       x=NULL,y="Yield (%)",color=NULL,
       caption=timestamp_caption())
...
rf_regime_metrics = rf_backtest %>%
  dplyr::group_by(target,curve_regime) %>%
  dplyr::summarise(
    observations=dplyr::n(),
    rf_mae=mean(abs(error)),
    persistence_mae=mean(abs(persistence_error)),
...
knitr::kable(
  rf_regime_metrics %>%
    dplyr::mutate(dplyr::across(where(is.numeric),~round(.x,3))),
  caption="Errors by yield-curve regime at the forecast origin."
)

```


## Legacy In-Sample US Diagnostics

These plots are retained for exploratory diagnosis only. They contain fitted
values from the full training sample and must not be interpreted as evidence of
forecast accuracy; the walk-forward results above are the governing validation.
...
df_predict_DFF %>%
  ...
  theme_money_printer_go_brrr(base_size=12) +
  labs(title="In-Sample Random-Forest Fit — Not Forecast Validation",
       x=NULL,
       y="(%)",
       caption = timestamp_caption()) +
...
df_predict_DFF %>%
  ...
  # facet_wrap(~one_year,scales="free",ncol=1) +
  theme_money_printer_go_brrr(base_size=12) +
  labs(title="In-Sample Random-Forest Fits — Not Forecast Validation",
       x=NULL,
       y="(%)",
       caption = timestamp_caption()) +
  add_fomc_meeting_dates()
...
df_predict_DFF %>%
  ...
  # facet_wrap(~one_year,scales="free",ncol=1) +
  theme_money_printer_go_brrr(base_size=12) +
  labs(title="Experimental One-Year Random-Forest Scenarios",
       x=NULL,
       y="(%)",
       caption = timestamp_caption()) +
  add_fomc_meeting_dates()
...
df_predict_DFF %>%
  ...
  # facet_wrap(~one_year,scales="free",ncol=1) +
  theme_money_printer_go_brrr(base_size=12) +
  labs(title="Experimental One-Year 2s10s Spread Scenario",
       x=NULL,
       y="(%)",
       caption = timestamp_caption()) +
  add_fomc_meeting_dates()
...
## Experimental One-Year Yield Scenario

This is a conditional model scenario, not a market price or a promise. The
random-forest estimate is shown beside persistence, with an empirically
calibrated 80% interval from earlier walk-forward errors. A model with
non-positive out-of-sample skill should not be preferred to persistence.
...
final_fits = list(
  ...
  DGS2=fit_DGS2,
  DGS10=fit_DGS10,
  MORTGAGE30US=fit_MORTGAGE30US
)

rf_live_forecast = dplyr::bind_rows(lapply(names(rf_targets),function(target_name) {
  target_spec=rf_targets[[target_name]]
  latest=target_spec$data %>%
    dplyr::filter(stats::complete.cases(dplyr::select(.,-date,-value))) %>%
    dplyr::filter(date==max(date))
  interval_width=rf_backtest %>%
    dplyr::filter(target==target_name) %>%
    dplyr::summarise(value=stats::quantile(abs(error),0.80,na.rm=TRUE,type=8)) %>%
    dplyr::pull(value)
  estimate=as.numeric(predict(final_fits[[target_name]],latest))

  data.frame(
    target=target_name,
    origin=latest$date,
    forecast_date=latest$date+years(1),
    random_forest=estimate,
    persistence=latest[[target_spec$current]],
    lower_80=estimate-interval_width,
    upper_80=estimate+interval_width
  )
})) %>%
  dplyr::left_join(dplyr::select(rf_metrics,target,rmse_skill),by="target") %>%
  dplyr::mutate(preferred_estimate=ifelse(rmse_skill>0,random_forest,persistence),
                target=factor(target,levels=c("DFF","DGS2","DGS10","MORTGAGE30US")))

knitr::kable(
  rf_live_forecast %>%
    dplyr::mutate(dplyr::across(c(random_forest,persistence,lower_80,upper_80,rmse_skill),~round(.x,3))),
  caption="Experimental one-year scenario. Preferred estimate uses the forest only when it beat persistence out of sample."
)

rf_live_forecast %>%
  ggplot(.,aes(x=target,y=preferred_estimate,group=1))+
  geom_ribbon(aes(ymin=lower_80,ymax=upper_80,group=1),alpha=0.15)+
  geom_line(linewidth=1)+
  geom_point(size=3)+
  geom_point(aes(y=persistence),shape=4,size=3)+
  scale_y_continuous(breaks=seq(-2,12,0.5))+
  theme_money_printer_go_brrr(base_size=20) +
  labs(title="Historical Random-Forest Scenario Based on the Current Yield Curve",
       subtitle="One year ahead; crosses show persistence and shading shows calibrated 80% RF error intervals",
       x=NULL,y="Yield (%)",
       caption = timestamp_caption())

```

## Experimental FOMC Meeting Scenario

```{r }

dff_rf_has_skill = rf_metrics %>%
  dplyr::filter(target=="DFF") %>%
  dplyr::pull(rmse_skill) > 0
...
df_predict_DFF %>%
  dplyr::mutate(pred_DFF=predict(fit_DFF,.),
                ...
                pred_DGS10=predict(fit_DGS10,.),
                pred_MORTGAGE30US=predict(fit_MORTGAGE30US,.),
                pred_DFF=ifelse(dff_rf_has_skill,pred_DFF,level_DFF)) %>%
  dplyr::select(date,pred_DFF:pred_MORTGAGE30US) %>%
  dplyr::mutate(date=date+years(1)) %>%
  gather(pred,val,pred_DFF:pred_MORTGAGE30US) %>%
  filter(!is.na(date)) %>%
  ...
  theme_money_printer_go_brrr(base_size=12) +
  labs(title="Experimental FOMC Meeting Rate Scenario",
       subtitle=ifelse(dff_rf_has_skill,
                       "Random forest beat persistence in walk-forward testing",
                       "Persistence shown because random forest did not beat it out of sample"),
       x=NULL,
       y="(%)",
       caption = timestamp_caption())
...
## Legacy Danish Rate Diagnostics

These Danish fits have not yet received the walk-forward validation applied to
the US series above. Treat them as exploratory scenarios, not forecasts.
...
df_predict_DK_Short %>%
  ...
  geom_hline(yintercept = 0,linetype=2) +
  theme_money_printer_go_brrr(base_size=12) +
  labs(title="In-Sample Danish Rate Fit — Not Forecast Validation",
       x=NULL,
       y="(%)",
       caption = timestamp_caption())
...
df_predict_DK_Short %>%
  ...
  geom_hline(yintercept = 0,linetype=2) +
  theme_money_printer_go_brrr(base_size=12) +
  labs(title="Experimental One-Year Danish Rate Scenario",
       x=NULL,
       y="(%)",
       caption = timestamp_caption())
...
# Effective dates and target ranges from the Federal Reserve's official history:
# https://www.federalreserve.gov/monetarypolicy/openmarket.htm
fed_rate_changes = tibble::tribble(
  ~effective_date, ~target_lower, ~target_upper, ~change_bps,
  "2022-03-17", 0.25, 0.50,  25,
  "2022-05-05", 0.75, 1.00,  50,
  "2022-06-16", 1.50, 1.75,  75,
  "2022-07-28", 2.25, 2.50,  75,
  "2022-09-22", 3.00, 3.25,  75,
  "2022-11-03", 3.75, 4.00,  75,
  "2022-12-15", 4.25, 4.50,  50,
  "2023-02-02", 4.50, 4.75,  25,
  "2023-03-23", 4.75, 5.00,  25,
  "2023-05-04", 5.00, 5.25,  25,
  "2023-07-27", 5.25, 5.50,  25,
  "2024-09-19", 4.75, 5.00, -50,
  "2024-11-08", 4.50, 4.75, -25,
  "2024-12-19", 4.25, 4.50, -25,
  "2025-09-18", 4.00, 4.25, -25,
  "2025-10-30", 3.75, 4.00, -25,
  "2025-12-11", 3.50, 3.75, -25
) %>%
  dplyr::mutate(effective_date = ymd(effective_date))

fomc_decisions = do.call("rbind",list(
data.frame(type="hike",
           start_date=c("1994-02-04","1997-03-25","1999-06-30","2004-06-30","2015-12-17","2022-03-17")),
data.frame(type="cut",
           start_date=c("1990-07-13","1995-07-06","1998-09-29","2001-01-03","2002-11-06","2007-09-18","2008-10-08","2019-08-01","2020-03-03","2024-09-19"))
)) %>%
...
```{r}
# Deposit Rate
df_ECB_DepositRate = get_ecb_data(key = glue("FM.D.U2.EUR.4F.KR.DFR.LEV"))   %>%
...
df_ECB_yields = do.call("rbind",lapply(yields,function(yield) {
  get_ecb_data(key = glue("YC.B.U2.EUR.4F.G_N_A.SV_C_YM.SR_{yield}"))  %>%
  dplyr::mutate(obstime=ymd(obstime),
                Yield=yield) %>%
    dplyr::select(Yield,obstime,obsvalue)
}))
...
## Legacy ECB Yield Model

This ECB model remains an experimental scenario and has not yet passed the
walk-forward benchmark used for the US model.
...
df_DK10Y = get_ecb_data(key = "IRS.M.DK.L.L40.CI.0000.DKK.N.Z")  %>%
  dplyr::mutate(obstime=ymd(paste0(obstime,"-01")),
                Yield="10Y",
                dk_obsvalue=obsvalue/100) %>%
    dplyr::select(Yield,obstime,dk_obsvalue)

df_DK3MO = get_ecb_data(key = "FM.M.DK.DKK.DS.MM.CIBOR3M.ASKA")  %>%
  dplyr::mutate(obstime=ymd(paste0(obstime,"-01")),
                Yield="3M",
                dk_obsvalue=obsvalue/100) %>%
    dplyr::select(Yield,obstime,dk_obsvalue)
