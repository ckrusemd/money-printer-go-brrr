...
if (!is.na(current_url) && nzchar(current_url)) {
  ...
  df_interest = tryCatch({
    interest = openxlsx::read.xlsx(xlsxFile = xlsx_url, startRow = 1)
    interest$År[1] = 1997
    interest = interest %>% fill(År, .direction = "down")
    interest$Date = as.Date(paste(interest$År, interest$Uge, 1, sep="-"), "%Y-%U-%u")
    interest %>%
