...
```{r}
series_ids_yahoo = unique(c("AAPL","JNJ","MSFT"))
...
calc_ccf = function(data) {
  ccf_results = ccf(data$value.x,data$value.y,lag.max = 3650,plot = FALSE)
...
df.extrapolate$risk = predict(fit.glm,df.extrapolate,type = "response") %>% pull(Yes) 
