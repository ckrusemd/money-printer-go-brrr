...
prophoet.faerdigt_byggeri =  prophet( df.faerdigt_byggeri , seasonality.mode = 'multiplicative')

future <- make_future_dataframe(include_history = TRUE,m = prophet.faerdigt_byggeri, periods = 5*365,freq = "days")
forecast <- predict(prophoet.faerdigt_byggeri, future)

forecast %>% 
  dplyr::mutate(yhat=yhat) %>% 
  ggplot(.,aes(x=ds,y=yhat)) +
  geom_line(alpha=0.3,color="blue") +
  geom_line(data=df.faerdigt_byggeri,aes(x=as.POSIXct(ds),y=y))
