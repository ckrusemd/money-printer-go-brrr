...
gam_adjust = function(data) {
    ...
    # Statistics Denmark can return groups with missing, duplicated, or
    # constant dates. Fit only on finite, unique observations and fall back
    # to the original values if a spline cannot be estimated.
    spline_rows <- is.finite(as.numeric(data$DATE)) & is.finite(data$VALUE)
    spline_data <- data[spline_rows, , drop = FALSE]
    fit_smooth <- NULL
    if (nrow(spline_data) >= 4 && length(unique(spline_data$DATE)) >= 4) {
      fit_smooth <- tryCatch(
        smooth.spline(as.numeric(spline_data$DATE), spline_data$VALUE, cv = TRUE),
        error = function(e) NULL
      )
    }
    value_smooth <- data$VALUE
    if (!is.null(fit_smooth)) {
      value_smooth[spline_rows] <- predict(
        fit_smooth, x = as.numeric(spline_data$DATE)
      )$y
    }
    ema_n <- min(20, nrow(data))
    value_ema <- if (ema_n > 0) {
      tryCatch(TTR::EMA(data$VALUE, ema_n), error = function(e) data$VALUE)
    } else {
      numeric(0)
    }
  ...
  data %>% 
    ...
    dplyr::mutate(VALUE_SMOOTH=value_smooth) %>%
    ...
    dplyr::mutate(VALUE_EMA=value_ema) %>%
    arrange(desc(DATE)) %>%
...
# dkstat requests code-and-label values when a table has duplicate labels.
# Normalize the categorical columns so downstream filters use stable labels.
strip_statbank_codes <- function(data, columns) {
  data %>%
...
BYGV80 <- dst_get_data(table = "BYGV80", 
                       ...
                       Tid="*",
                       lang = "da") %>% 
  strip_statbank_codes(c("BYGFASE", "ANVENDELSE")) %>%
...
BYG1 <- dst_get_data(table = "BYG1", 
                       ...
                       Tid="*",
                       lang = "da") %>% 
  strip_statbank_codes(c("BRANCHE07", "SÆSON", "ART")) %>%
  dplyr::rename(DATE=TID) %>% 
  dplyr::mutate(DATE=as.Date(DATE)) %>%
  dplyr::rename(VALUE=value) %>% 
...
PRIS90 <- dst_get_data(table = "PRIS90", 
                       ...
                       Tid="*",
                       lang = "da") %>% 
  strip_statbank_codes(c("ENHED", "BOLTYP")) %>%
  dplyr::rename(DATE=TID) %>% 
  dplyr::mutate(DATE=as.Date(DATE)) %>%
  dplyr::rename(VALUE=value) %>% 
  filter(ENHED=="Indeks") %>% 
...
BYG42 <- dst_get_data(table = "BYG42", 
                       ...
                       Tid="*",
                       lang = "da") %>% 
  strip_statbank_codes(c("HINDEKS", "DINDEKS", "ART", "TAL")) %>%
  dplyr::rename(DATE=TID) %>% 
  dplyr::mutate(DATE=as.Date(DATE)) %>%
  dplyr::rename(VALUE=value) %>% 
...
construction_data <- BYGV80 %>%
  filter(PROPERTY_TYPE=="Parcel-/rækkehuse")

if (nrow(construction_data) == 0) {
  ggplot() +
    theme_void() +
    labs(title = "Construction activity data unavailable")
} else {
  construction_data %>%
...
if (nrow(flats_data) == 0) {
  ggplot() +
    theme_void() +
    labs(title = "Flats construction data unavailable")
} else {
  flats_data %>%
