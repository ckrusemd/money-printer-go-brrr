...
gam_adjust = function(data) {
    ...
    fit_smooth = tryCatch(
      smooth.spline(data$DATE, data$VALUE, cv = TRUE),
      error = function(error) {
        warning("Smoothing skipped for sparse or invalid group: ", error$message)
        NULL
      }
    )
    smooth_values = if (is.null(fit_smooth)) data$VALUE else fit_smooth$y
    ema_values = if (nrow(data) > 0) {
      TTR::EMA(data$VALUE, n = min(20, nrow(data)))
    } else {
      numeric()
    }
  ...
  data %>% 
    ...
    dplyr::mutate(VALUE_SMOOTH=smooth_values) %>%
    ...
    dplyr::mutate(VALUE_EMA=ema_values) %>%
    arrange(desc(DATE)) %>%
...
BYGV80 <- dst_get_data(table = "BYGV80", 
  ...
  dplyr::mutate(PROPERTY_TYPE = dplyr::case_when(
    grepl("etage|ejer|lejlig|flat", ANVENDELSE, ignore.case = TRUE) ~ "Ejerlejlighed",
    grepl("parcel|række|enfamil|house|hus", ANVENDELSE, ignore.case = TRUE) ~ "Parcel-/rækkehuse",
    TRUE ~ NA_character_
  )) %>%
...
construction_data = BYGV80 %>% filter(PROPERTY_TYPE=="Parcel-/rækkehuse")
if (nrow(construction_data) == 0) {
  cat("Construction series unavailable from Statistics Denmark for this run.")
} else {
construction_data %>%
...
```{r}
if (nrow(construction_data) == 0) {
  cat("No construction series available for the cumulative-change chart.")
} else {
construction_data %>%
...
flats_data = BYGV80 %>% filter(PROPERTY_TYPE == "Ejerlejlighed")
if (nrow(flats_data) == 0) {
  cat("No apartment construction series available for this run.")
} else {
flats_data %>%
