...
ccf_results = ccf(df_ccf_BYGV80$`Tilladt byggeri`,df_ccf_BYGV80$`Fuldført byggeri`,lag.max = 1000,plot = FALSE)
  plot(ccf_results)
...
ccf_results = ccf(df_ccf_BYG1$`Fuldført byggeri`,df_ccf_BYG1$`I alt`,lag.max = 1000,plot = FALSE)
  plot(ccf_results)
  best_corr_index = which(abs(ccf_results$acf)==max(abs(ccf_results$acf)))
  ccf_results$lag[best_corr_index]
  ccf_results$acf[best_corr_index]
