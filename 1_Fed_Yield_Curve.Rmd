...
```{r }
fredr::fredr_set_key(key = "XXXXXX")
```
...
df_yieldcurve %>% 
  ...
  spread(series_id,value) %>% 
  dplyr::mutate(spread=MORTGAGE30US-DGS30) %>% 
...
df_yieldcurve %>% 
  ...
  spread(series_id,value) %>% 
  dplyr::mutate(spread=DGS10-DGS2) %>% 
  ggplot(.,aes(x=date,y=spread)) +
  geom_line() +
  theme(legend.position="bottom")
```

## DGS2-MORTGAGE30 Spread
```{r}
...
df_yieldcurve %>% 
  ...
  spread(series_id,value) %>% 
  dplyr::mutate(spread=MORTGAGE30US-DGS6MO) %>% 
  ggplot(.,aes(x=date,y=spread)) +
  geom_line() +
  theme(legend.position="bottom")
```
...
```{r}
ggplot(df_yieldcurve,aes(x=date,y=value)) +
...
ggplot(df_yieldcurve,aes(x=date,y=value)) +
  geom_line(aes(group=series_id,color=series_id)) +
  theme(legend.position="bottom") +
  scale_x_date(limits=c(Sys.Date()-years(1),NA)) +
