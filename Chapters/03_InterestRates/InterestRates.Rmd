...
df_predict_M2SL = df_yield_checkboard %>% 
  ...
  spread(series_id,spread) %>% 
  arrange(desc(date))
df_predict_WILL5000INDFC = df_yield_checkboard %>% 
  filter(series_id!=joined_series) %>% 
  dplyr::mutate(one_year_ahead=date+years(1)) %>% 
  left_join(df_WILL5000INDFC,by=c("one_year_ahead"="date")) %>% 
  unite(series_id,series_id,joined_series) %>% 
  dplyr::select(date,series_id,spread,value) %>% 
  spread(series_id,spread) %>% 
  arrange(desc(date))
...
fit_M2SL = caret::train(x = df_predict_M2SL %>% 
                         ...
                         metric = "RMSE",
                         trControl=train_control)
fit_WILL5000INDFC = caret::train(x = df_predict_WILL5000INDFC %>% 
                              na.omit %>% 
                              dplyr::select(-date,-value) %>% 
                              data.frame(),
                         y = df_predict_WILL5000INDFC %>% 
                           na.omit %>% 
                           .[["value"]],
                         method = "rf",
                         tuneGrid=tune_grid,
...
df_predict_WILL5000INDFC %>% 
    dplyr::mutate(pred_WILL5000INDFC=predict(fit_WILL5000INDFC,.)) %>% 
  dplyr::select(date,pred_WILL5000INDFC,value) %>% 
  dplyr::mutate(date=date+years(1)) %>%
  gather(pred,val,pred_WILL5000INDFC:value) %>%
  tidyr::crossing(one_year=c(TRUE,FALSE)) %>% 
  dplyr::mutate(date=ifelse(one_year,
                            ifelse(date>=Sys.Date()-years(1),date,NA),date)) %>% 
  filter(!is.na(date)) %>% 
  ...
  # facet_wrap(~one_year,scales="free",ncol=1) +
  theme_money_printer_go_brrr(base_size=12) +
  labs(title="Predicted/Observed YoY Willshire",
       x=NULL,
       y="(%)",
       caption = timestamp_caption())
